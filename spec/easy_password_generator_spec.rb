...
require "spec_helper"
require "easy_password_generator"

RSpec.describe EasyPasswordGenerator do
  it "has a version number" do
    expect(EasyPasswordGenerator::VERSION).not_to be nil
  end
end

RSpec.describe EasyPasswordGenerator::Generator do
  describe ".generate" do
    context "with default settings" do
      ...
      it "generates unique passwords each time" do
        password1 = EasyPasswordGenerator::Generator.generate
        password2 = EasyPasswordGenerator::Generator.generate
    ...
    context "with custom settings" do
      it "generates a password of specified length" do
        password = EasyPasswordGenerator::Generator.generate(16)
        expect(password.length).to eq(16)
      end

      it "excludes upper case letters when upper_case is false" do
        password = EasyPasswordGenerator::Generator.generate(12, upper_case: false)
        expect(password).not_to match(/[A-Z]/)
      end

      it "excludes lower case letters when lower_case is false" do
        password = EasyPasswordGenerator::Generator.generate(12, lower_case: false)
        expect(password).not_to match(/[a-z]/)
      end

      it "excludes numbers when numbers is false" do
        password = EasyPasswordGenerator::Generator.generate(12, numbers: false)
        expect(password).not_to match(/[0-9]/)
      end

      it "includes symbols when symbols is true" do
        password = EasyPasswordGenerator::Generator.generate(12, symbols: true)
        expect(password).to match(/[!@#$%^&*\-_+=]/)
      end

      it "raises an error if all character sets are disabled" do
        expect do
          EasyPasswordGenerator::Generator.generate(12, upper_case: false, lower_case: false, numbers: false,
                                                    symbols: false)
    ...
    context "with known keywords" do
      ...
      it "handles keywords longer than the specified length" do
        password = EasyPasswordGenerator::Generator.generate(5, known_keywords: "elephant", mix: false)
      ...
      it "generates a password with exact length when using only known keywords" do
        password = EasyPasswordGenerator::Generator.generate(15, known_keywords: "alpha,beta,gamma", mix: false)
