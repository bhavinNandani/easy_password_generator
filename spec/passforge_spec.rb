...
RSpec.describe PassForge do
  it "has a version number" do
    expect(PassForge::VERSION).not_to be nil
  end
end

RSpec.describe PassForge::Generator do
  describe ".generate" do
    context "with default settings" do
      it "generates a password of default length 12" do
        password = PassForge::Generator.generate
      ...
      it "generates unique passwords each time" do
        password1 = PassForge::Generator.generate
        password2 = PassForge::Generator.generate
    ...
    context "with custom settings" do
      ...
      it "raises an error if all character sets are disabled" do
        expect do
          PassForge::Generator.generate(12, upper_case: false, lower_case: false, numbers: false,
                                                    symbols: false)
    ...
    context "with known keywords" do
      it "generates a password using only known keywords when mix is false" do
        password = PassForge::Generator.generate(12, known_keywords: "dog,cat,fish", mix: false)
        expect(password.length).to eq(12)
      ...
      it "generates a mixed password with known keywords and random characters when mix is true" do
        password = PassForge::Generator.generate(12, known_keywords: "dog,cat,fish", mix: true)
        expect(password.length).to eq(12)
        known_keywords = %w[dog cat fish]
        expect(known_keywords.any? { |kw| password.include?(kw) }).to be true
      ...
      it "handles keywords longer than the specified length" do
        password = PassForge::Generator.generate(5, known_keywords: "elephant", mix: false)
      ...
      it "generates a password with exact length when using only known keywords" do
        password = PassForge::Generator.generate(15, known_keywords: "alpha,beta,gamma", mix: false)
